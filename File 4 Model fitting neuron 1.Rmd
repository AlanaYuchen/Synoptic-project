---
title: "Model Fitting Neuron 1 (n200819)"
author: "Alana"
date: "2022/5/25"
output: pdf_document
---
...
# Parameter fitting
The firing rates of  n200819 neuron is 3.39 spike/s.According to the simulation, the approximated input rate Ire for it is 267.
```{r}
ID=1
Ire=267
```
...
for(kHAP in kHAPs){
  ...
  ds=cbind(rep(paste0('kHAP_',kHAP),length(spiketime)),
           rep(paste0('lambda_HAP_',lambda_HAP),length(spiketime)),spiketime)
...
for(kHAP in kHAPs){
  for(lambda_HAP in lambda_HAPs){
  spiketimedt=HAP_fit[which(HAP_fit$kHAP==paste0('kHAP_',kHAP)&
                          HAP_fit$lambda_HAP==paste0('lambda_HAP_',lambda_HAP)),]
  ...
  ds=cbind(rep(paste0('kHAP_',kHAP),length(ISI)),
           rep(paste0('lambda_HAP_',lambda_HAP),length(ISI)),ISI)
...
```{r}
library(ggplot2)
HAP_hazard=data.frame()
for(kHAP in kHAPs){
  for(lambda_HAP in lambda_HAPs){
    celldt=HAP_fit[which(HAP_fit$kHAP==paste0('kHAP_',kHAP)&
                           HAP_fit$lambda_HAP==paste0('lambda_HAP_',lambda_HAP)),]
    spikecount=length(celldt[,1])
    ISIdt=HAP_fit_ISI[which(HAP_fit_ISI$V1==paste0('kHAP_',kHAP)&
                              HAP_fit_ISI$V2==paste0('lambda_HAP_',lambda_HAP)),]
    plothist=hist(ISIdt$ISI,breaks=seq(0,max(ISIdt$ISI)+0.005,0.005),plot = F)
    ...
    hazarddt=cbind(rep(paste0('kHAP_',kHAP,'-lambda_HAP_',lambda_HAP),length(haz)),time,haz)
    HAP_hazard=rbind(HAP_hazard,hazarddt)
  }
}
colnames(HAP_hazard)[1]='Cell_ID'
HAP_hazard$time=as.numeric(as.character(HAP_hazard$time))
HAP_hazard$haz=as.numeric(as.character(HAP_hazard$haz))
  #plot(c(1:bincount)*0.005,haz)
plothazard=ggplot(HAP_hazard,aes(x=time,y=haz,color=Cell_ID))+
    geom_point(size=0.5,alpha=0.6)+
    theme_classic()+labs(title = 'Model')+xlab('Time (s)')+ylab('Hazard')+
    xlim(0,0.5)+ylim(0,0.03)
```
...
for(kHAP in kHAPs){
  for(lambda_HAP in lambda_HAPs){
    modeldt=HAP_fit_ISI[which(HAP_fit_ISI$Cell_ID==
                                paste0('kHAP_',kHAP,'-lambda_HAP_',lambda_HAP)),3:4]
...
For Cell 1, or desity plot fitting, k and lambda together, k=20, lambda=8

## Plot fitted HAP parameters
```{r}
library(ggplot2)
ISI_combine=rbind(Cell_ISI[which(Cell_ISI$Cell_ID==Cell_ID[ID]),2:3],HAP_fit_ISI[,3:4])
ISI_combine=rbind(Cell_ISI[which(Cell_ISI$Cell_ID==Cell_ID[ID]),2:3],
                  HAP_fit_ISI[which(HAP_fit_ISI$Cell_ID=='kHAP_20-lambda_HAP_8'),3:4])
# ggplot(ISI_combine,aes(x=ISI,color=Cell_ID))+
# geom_histogram(binwidth = 0.005)+xlim(-0.1,1)+theme_classic()+
...
```{r}
# Hazard_combine=rbind(Cell_Hazard[which(Cell_Hazard$Cell_ID==Cell_ID[ID]),2:4],HAP_hazard)
Hazard_combine=rbind(Cell_Hazard[which(Cell_Hazard$Cell_ID==Cell_ID[ID]),2:4],
                     HAP_hazard[which(HAP_hazard$Cell_ID=='kHAP_35-lambda_HAP_8.5'),])
ggplot(Hazard_combine,aes(x=time,y=haz,color=Cell_ID))+
    geom_point(size=0.5,alpha=0.6)+
    geom_smooth(se=F)+
    theme_classic()+labs(title = 'Model')+xlab('Time (s)')+ylab('Hazard')+
