---
title: "Model fitting Cell n210211"
author: "Alana"
date: "2022/5/26"
output: pdf_document
---
...
# Parameter fitting
The firing rates of  n210211 neuron is 9.24 spike/s.According to the simulation, the approximated input rate Ire for it is 585.
```{r}
ID=4
Ire=585
```
...
RMSE4=RMSE
```
For Cell 4, the best fitting parameters are kHAP=20, lambda_HAP=9

## Fitting plots
```{r}
library(ggplot2)
ISI_combine=rbind(Cell_ISI[which(Cell_ISI$Cell_ID==Cell_ID[ID]),2:3],HAP_fit_ISI[,3:4])
ISI_combine=rbind(Cell_ISI[which(Cell_ISI$Cell_ID==Cell_ID[ID]),2:3],HAP_fit_ISI[which(HAP_fit_ISI$Cell_ID=='kHAP_20-lambda_HAP_9'),3:4])
